...
Parse `.icvs` string into an AST `Document`.
...
Parse a `.icvs` file from the filesystem.

### `validator::validate(doc: &Document) -> Result<ValidationReport>`

Validate DAG structure, detect cycles, orphan nodes, unresolved references.
...
Kahn's algorithm. Returns error if a cycle exists.
...
Parse + resolve includes in one step.

### `exporter::export_markdown(doc: &Document, target: &str) -> Result<String>`

Export Markdown for a specific target (applies resolve/ignore filtering).

### `exporter::export_dot(doc: &Document) -> Result<String>`

Export DOT graph with per-node-type colors.

### `exporter::export_markdown_merge(doc: &Document) -> Result<String>`

Export all nodes without target filtering.
...
| `parseAndResolve(input)` | `string` | `JSON string` | Parse + resolve includes |
...
| `mdToIcvs(markdown)` | `string` | `string` | Convert Markdown → .icvs |
| `icvsToMd(input)` | `string` | `string` | Convert .icvs → Markdown |
| `convertAgent(input, format)` | `string, string` | `string` | Export as Claude/OpenAI/JSON |
| `applyTemplate(input, varsJson)` | `string, string` | `string` | Apply `{{ VAR }}` template |
